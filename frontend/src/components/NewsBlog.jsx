...
const Card = ({ category, title, description, author, date, imageUrl, profile, link }) => {
    return (
        <div className="bg-white rounded-lg shadow-lg overflow-hidden w-full md:w-80 duration-500 hover:scale-105 hover:shadow-xl" id='newsblog'>
            <img
        ...
        <div className='px-4 md:px-14 py-16 max-w-screen-2xl mx-auto' id='blog'> {/* Flex container */}
            ...
            <div className="text-end mt-4 text-blue-700">
                <button className='text-green-600 hover:text-green-500' onClick={toggleVisibility}>
                    <Link to={"/morenewsblog"}>{visibility ? 'Back' : <span className='flex items-center '>More <MdKeyboardDoubleArrowRight className='text-2xl' /></span>}</Link>
                </button>
            </div>
        </div>
    );
