...
const MoreAgroEquip = () => {
  ...
  const blogs = [
    {
      id: 1, title: "Tractor", description: "A versatile machine used for pulling or pushing agricultural machinery, such as plows, cultivators, trailers, etc.",
  ...
  return <>
    ...
    <div className='px-4 lg:px-14 max-w-screen-2xl mx-auto my-12' id='blog'>
      ...
      <div className='grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-8 items-center justify-between'>
        {
          blogs.map(blog => <div key={blog.id} data-aos="zoom-out-up"
            ...
            <a href={blog.link} target='_blank'><div className='text-center px-4 py-8 bg-white shadow-lg rounded-md md:w-3/4  mx-auto absolute left-0 right-0 -bottom-12'>
      ...
      <div className="text-end mt-4 text-green-600 hover:text-green-500">
        <button >
          <Link to="/?section=agroequip">{visibility ? <span className='flex items-center '><MdKeyboardDoubleArrowLeft className='text-2xl' /> Back </span> : ' More...'}</Link>
        </button>
      </div>
